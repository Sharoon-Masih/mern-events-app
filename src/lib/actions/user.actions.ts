...
export async function updateUser(clerkId: string, user: UpdateUserParams) {
    ...
    try {
        ...
        await connectToDb();
        const updatedUser = await User.findOneAndUpdate({ clerkId }, user, { new: true }); //yaha par by using "findOneAndUpdate" method usme jo 1st parameter hai usko clerkId ki base pa user ka doc ko find krega and then usme "user" ka jo updated obj mila wo pass krdia.
...
export async function getUser(userId: string) { //ab jo yeh wala action hna yeh clerk ki jo webhook bnai hai usme call nhi hoga bcuz waha par humay User get thori krwana hai waha say toh just user ko create krwana hai ya user ka data ma koi be mutation ho wo update and delete.
    ...
    try {
        ...
        await connectToDb();
        const getUser = await User.findById(userId) //yaha par .findById or findOne ma yeh difference haka findOne jo be doc ma field detay hain uska A/c filter kta hai or jo findById hai wo jo doc ki "_id" hoti hai joka tab bnti hai jab doc create hota hai toh uska A/c filter krta hai.
